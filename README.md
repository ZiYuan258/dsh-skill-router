...
- **固定目录成本，不随库增长。** 三个工具的 schema 合计 **3,603 B ≈ 1,001 token/轮**（实测，与会话长短无关）；技能库本身从不注入上下文。
- **由 agent 选。** 判断规则写在工具描述里，由模型决定用哪些技能——不把选择推给用户。

### 成本要说实话

工具驱动不是免费的，它有**两笔**开销，README 不该只讲省下的那笔：

| 开销 | 实测值 | 说明 |
|---|---|---|
| 常驻 schema | **3,603 B ≈ 1,001 token/轮** | 三个工具的 description + parameters，每轮请求都付。**固定**，不受技能库大小影响 |
| 一次检索往返 | 模型生成 1 次 tool call + 返回体积（`limit=12` 时约 **1,533 B ≈ 426 token**） | 与"目录已注入、模型直接挑"相比，这是为了拿到目录之外的能力而多付的一步 |

所以省往返最直接的两个动作是：**把 `limit` 调小**（或 `names_only: true`，返回体积约降 60%），**以及确定名字时直接 `skill_load`**，跳过检索。

对照：常驻 28 个技能时实测 **1,541 token/轮**——把 1,541 + 3,603 = 5,144 B 换成"只留 6 个常驻 + 工具"，净额才划算。**如果你的技能库很小（十几个），这个插件不划算**，直接用常驻目录更省。
...
需要 DSH 与一个 profile。`dsh.engines.dsh` 声明 `>=0.1.5-rc.1`——那是**已验证可用的版本**，不是"需要这么新"：本插件只用到 `ctx.tools.register` 与 `ctx.fs.*` 这一小组 API，无事件钩子、无 import；更早的版本我没有验证过，**不想过度声明兼容性**。`ctx.get` / `ctx.effect` / `ctx.skills` 全部是**可选**的，缺失时降级而不是崩溃（由 `test/minimal-host.mjs` 钉住）。`dsh plugin` 会在 profile 目录里转发给 `pnpm`，并同步 bundle 列表。
